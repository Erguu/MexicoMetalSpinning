...
| ITEM-46 | **RESOLVED 2026-08-09** (branch `fix/cylinder-idle-and-drive-power`, not compiled) | SheetHolder retract coil energised forever at idle — `bSheetHolderRetractHold` is now released by `tonSheetHolderHold` and `Timeout_Retract` is T#24H, so the FB never reaches the State-4 `Sol_B` dead end. Same session also fixed the re-extend-on-stop bug, the dead BackSupport end-retract on the stop path, the STOPPED contactor drop that cleared `HomingDone`, and `DB_MachineConfig` retentivity. |
