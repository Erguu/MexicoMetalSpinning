...
STATE_CYL_GOTO_WAIT(71)→ CMD=40: wait for BackSupport AtSetpoint or error (16#0309)
...
STATE 18   STOPPING             → Halt recipe → set bMandrelRetractPulse → LOCK_RETRACT_WAIT → STOP_GOHOME
...
STATE 29   LOCK_RETRACT_WAIT    → ToolHeadLock releasing (T#5S spring-return wait); exits to STOP_GOHOME or TOOL_CHANGE
