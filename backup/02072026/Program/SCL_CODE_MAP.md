...
STATE_PAUSED(800)      → Halt + wait for resume (returns to pauseReturnState: EXEC for motion, or back to dwell/spindle-wait)
...
| `FB_AlarmManager` | middle section | Consumes DB_SystemEvents queue, first-error latch on DB_Error.Code (secondary errors go to history only), updates DB_Error + DB_HMI_Errors |
